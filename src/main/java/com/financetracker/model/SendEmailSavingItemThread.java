...
public class SendEmailSavingItemThread implements Runnable {
				...
				for (User user : new UserDAO().getAllUsers()) {
					...
					for (BudgetItem budgetItem : budgetItems) {
						if (budgetItem.isExpOrInc()) {
							if(budgetItem.getCategory().equals("Deposits")){
								savings += budgetItem.getPayedMoney();
							}
						}
					}

					TreeSet<SavingItem> savingItems = new SavingItemDAO().getSavingItemsByUserId(userID);
					for (SavingItem savingItem : savingItems) {
						if(savingItem.getPrice() <= savings){
							this.sendEmail(user, savingItem);
						}
					}
	...
	private void sendEmail(User user, SavingItem savingItem) {
		...
		try {
			...
			message.setText("Hello " + user.getFirstName() + " " + user.getLastName() + " , \n"
					+ "You can now afford buying" + savingItem.getName() + " \n"
					+ "Best regards, \n FinanceTracker");
			// send the message
